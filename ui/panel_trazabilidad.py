...
evaluación normal) o en qué evaluación se evaluó cada criterio (en
FINAL). Usa las mismas funciones de cálculo que la exportación a Excel,
así que siempre coincide con lo que se ve en el archivo exportado.
"""
...
TEXTO_AYUDA_TRAZABILIDAD_EVALUACION = (
    ...
    "esta evaluación (columnas), si ese instrumento evalúa ese criterio y con qué peso.\n\n"
    "El peso se calcula automáticamente: es el peso del criterio en la materia, "
    "redistribuido entre los criterios marcados en ese mismo instrumento para que sumen "
    "100% entre ellos. No se puede editar a mano.\n\n"
    "Es la misma información que aparece en la hoja «Trazabilidad» del Excel exportado, "
    "pero aquí la puedes consultar directamente sin tener que exportar nada."
)

TEXTO_AYUDA_TRAZABILIDAD_FINAL = (
    "Esta tabla muestra, para cada criterio (filas) y cada evaluación 1EVA/2EVA/3EVA "
    "(columnas), si ese criterio tuvo alguna nota calculada en esa evaluación. La última "
    "columna cuenta en cuántas evaluaciones se trabajó cada criterio.\n\n"
    "Es la misma información que aparece en la hoja «Trazabilidad» del Excel exportado de "
    "FINAL, pero aquí la puedes consultar directamente sin tener que exportar nada."
)


class PanelTrazabilidadEvaluacion(QWidget):
    """Trazabilidad criterio <-> instrumento, para una evaluación normal."""
    ...
    def refrescar(self):
        criterios, instrumentos, pesos = self.base_datos.trazabilidad_criterios_instrumentos(
        ...
        for fila, criterio in enumerate(criterios):
            item_criterio = QTableWidgetItem(criterio.codigo)
            ...
            for col, instrumento in enumerate(instrumentos, start=1):
                ...
                if peso is None:
                    ...
                    item = QTableWidgetItem(f"Sí ({peso:g})")
...
class PanelTrazabilidadFinal(QWidget):
    ...
    def refrescar(self):
        criterios, evaluaciones, evaluado = self.base_datos.trazabilidad_criterios_evaluaciones(
            self.materia.id
        )
