...
toda la clase de golpe), en PDF o en Word, para una evaluación normal o
para FINAL.
"""
...
class DialogoGenerarInformes(QDialog):
    ...
    pasen al construirlo (distintas para evaluación normal y FINAL).
    """
    ...
    def __init__(
        ...
        self.combo_alumno.setMinimumHeight(32)
...
def generar_informe_evaluacion_individual(
    ...
    informe = recopilar_informe_evaluacion(base_datos, materia, evaluacion, alumno_id)
    if informe is None:
        return None
...
def generar_informe_final_individual(
    base_datos: BaseDatosCurso, materia: Materia, alumno_id: int, formato: str, carpeta_destino: Path,
) -> Path | None:
    informe = recopilar_informe_final(base_datos, materia, alumno_id)
    if informe is None:
        return None
    nombre_base = _nombre_archivo_seguro(
