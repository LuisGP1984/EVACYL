"""
Panel "Alumnos" de una materia: lista única de alumnado para toda la
materia (compartida por las 4 evaluaciones). Permite añadir uno a uno,
pegar directamente desde Excel (Ctrl+V) o importar un archivo .xlsx.
"""
...
NOMBRES_EVALUACION_POR_ORDEN = {1: "1EVA", 2: "2EVA", 3: "3EVA"}

TEXTO_AYUDA = (
    "En esta pestaña gestionas el alumnado de la materia (una sola lista, "
    "compartida por 1EVA, 2EVA, 3EVA y FINAL).\n\n"
    ...
    "Si un alumno o alumna se incorpora más adelante (en 2ª o 3ª evaluación), "
    "indícalo en la columna «Se incorpora en»: no contará en las evaluaciones "
...
class PanelAlumnos(QWidget):
    ...
    def __init__(self, base_datos: BaseDatosCurso, materia: Materia):
        ...
        titulo = QLabel("👥 Alumnado de la materia")
        ...
        ayuda = QLabel(
            "Esta lista es única para toda la materia. Las pestañas 1EVA, 2EVA, 3EVA y "
        ...
        # Pegar con Ctrl+V mientras la tabla tiene el foco
    ...
    def refrescar(self):
        ...
        for fila, alumno in enumerate(alumnos):
            ...
            combo = QComboBox()
            for orden, nombre_eval in NOMBRES_EVALUACION_POR_ORDEN.items():
                combo.addItem(nombre_eval, orden)
            combo.setCurrentIndex(alumno.orden_alta - 1)
    ...
    def anadir_alumno(self):
        self.base_datos.agregar_alumno(self.materia.id)
        self.refrescar()
    ...
    def _importar_lista(self, alumnos_nuevos: list[tuple[str, str]]):
        if not alumnos_nuevos:
            ...
            return
        insertados = self.base_datos.agregar_alumnos_en_lote(self.materia.id, alumnos_nuevos)
        self.refrescar()
