...
estructuras recopiladas en core/informe_alumno.py.
"""
...
COLOR_CABECERA = colors.HexColor("#0F6FB9")
COLOR_RESULTADO = colors.HexColor("#0D3D6B")
COLOR_FILA_ALTERNA = colors.HexColor("#EAF6F4")
...
def _estilo_tabla_estandar() -> TableStyle:
    return TableStyle(
        [
            ...
            ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#D7E3EA")),
...
def _cabecera_comun(story: list, nombre_materia: str, etiqueta_evaluacion: str, apellidos: str, nombre: str):
    ...
    story.append(Paragraph(f"<b>Materia:</b> {nombre_materia}", ESTILO_NORMAL))
...
def generar_informe_evaluacion_pdf(informe: InformeAlumno, ruta_destino: str | Path) -> Path:
    ...
    _cabecera_comun(story, informe.nombre_materia, etiqueta_evaluacion, informe.apellidos_alumno, informe.nombre_alumno)
    ...
    story.append(Paragraph("Calificación de cada criterio de evaluación", ESTILO_SUBTITULO))
    story.append(Spacer(1, 6))
    filas_tabla = [["Criterio", "Peso en la materia", "Nota", "Calificación"]]
...
def generar_informe_final_pdf(informe: InformeAlumnoFinal, ruta_destino: str | Path) -> Path:
    ...
    _cabecera_comun(
        story, informe.nombre_materia, _ETIQUETAS_EVALUACION["FINAL"],
    ...
    story.append(Spacer(1, 6))
    filas_tabla = [["Criterio", "1ª Ev.", "2ª Ev.", "3ª Ev.", "Nota FINAL", "Calif."]]
    for fila in informe.filas_criterios:
        filas_tabla.append(
            [
                fila.codigo_criterio,
    ...
    tabla = Table(filas_tabla, colWidths=[3 * cm, 2.3 * cm, 2.3 * cm, 2.3 * cm, 2.6 * cm, 2.5 * cm])
    tabla.setStyle(_estilo_tabla_estandar())
    story.append(tabla)
    story.append(Spacer(1, 14))
    story.append(
